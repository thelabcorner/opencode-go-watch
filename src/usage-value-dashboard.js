...
function bar(entry) {
  const width = typeof entry?.fractionOfBest === "number" && Number.isFinite(entry.fractionOfBest)
    ? Math.max(1.5, Math.min(100, entry.fractionOfBest * 100))
    : 0;
...
function band(entry, name) {
  const value = entry?.workload?.[name]?.requestsPerDollar;
  return amount(value);
}

function regime(entry) {
  if (entry?.regimes?.time) {
    return `Off ${amount(entry.regimes.time.offPeakRequestsPerDollar)} / $ · Peak ${amount(entry.regimes.time.peakRequestsPerDollar)} / $`;
  }
...
export function goUsageValueSection(snapshot) {
  ...
  return `<section class="section wrap" id="usage-value"><div class="head"><div><div class="kick">Usage Yield V2</div><h2>Most coding-agent usage for the Go subscription</h2><p>Paid models are normalized against the same observed OpenCode agent workload corpus, then ranked by standardized work available through each model's effective Go allowance.</p></div><a class="btn" href="/docs/usage-yield-v2" style="display:none">Method</a></div><div class="surface"><div class="surface-title"><strong>Usage value leaderboard</strong><span>${esc(calibrationSummary(ranking))}</span></div><div style="padding:12px 14px;border-bottom:1px solid #1c1c20;color:#71717a;font-size:9px;line-height:1.55">${best ? `Best paid value: <strong style="color:#d4d4d8">${esc(best.name)}</strong> · ${amount(best.goCapacity?.monthlyEquivalentRequests)} standardized monthly requests · ${amount(best.requestsPerDollar)} req/$` : "No paid model currently has complete V2 evidence."}${free.length ? ` · ${free.length} Go quota-exempt model${free.length === 1 ? "" : "s"} tracked separately` : ""}${unranked.length ? ` · ${unranked.length} unranked for incomplete evidence` : ""}</div><div class="tablewrap"><table class="table"><thead><tr><th class="num">Rank</th><th>Model</th><th class="num">Go monthly work</th><th class="num">Req / $</th><th class="num">$/standard request</th><th class="num">Vs best</th><th>Light / typical / heavy</th><th>Regime</th><th>Confidence</th></tr></thead><tbody>${rows || '<tr><td colspan="9">No ranked paid models.</td></tr>'}</tbody></table></div><div class="footerline"><span>Free ≠ unlimited. Quota-exempt Go states are not mixed into paid division-by-dollar ranking.</span><span>Rank recomputes on price, model, regime, or workload-corpus changes.</span></div></div></section>`;
}
